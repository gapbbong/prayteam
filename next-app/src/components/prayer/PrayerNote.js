...
export default function PrayerNote({
    ...
    const [addStatus, setAddStatus] = useState('idle'); // 'idle', 'saving', 'saved'
    ...
    const handleAddSubmit = async () => {
        ...
        try {
            // onAddPrayer now returns true/false (success)
            const success = await onAddPrayer(newPrayerText);
                            ...
                            className={`px-6 py-2 mt-1 rounded-2xl font-black text-white shadow-lg transition-all active:scale-95 flex items-center gap-2 whitespace-nowrap
                                ${!newPrayerText.trim()
                                    ...
                                    : addStatus === 'saved'
                                        ...
                                        : 'bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 dark:from-blue-700 dark:to-indigo-800'
                                }`}
                        >
                            {addStatus === 'saving' ? (
                                <>
